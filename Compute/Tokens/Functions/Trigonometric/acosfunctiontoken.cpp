...
void ACosFunctionToken::Operate(std::stack<std::shared_ptr<ValueToken>>& valueTokens,
                                const EvaluationSettings& settings) const
{
    if (GetNumberOfArguments() != 1)
        throw ComputeException("Invalid number of arguments for acos function");

    long double value {PopValue(valueTokens)};
    long double angle {std::acosl(value)};
